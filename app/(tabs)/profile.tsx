...
export default function ProfileScreen() {
  ...
  const handleLogout = (): void => {
    // In a real app, clear tokens and user data
  ...
  const renderProfileItem = (label: string, value: string, field: string, icon: string) => (
    <View className="py-4 ">
      <View className="flex-row justify-between items-center">
        <View className="flex-row items-center">
          <View className="w-8 h-8 rounded-full bg-secondary items-center justify-center mr-3">
            <Ionicons name={icon as any} size={18} color={isDark ? "#FBFBFB" : "#242424"} />
          </View>
      ...
      {editing === field ? (
          ...
          <View className="mt-2">
            <TextInput
              ...
              onChangeText={text => {
                ...
                else if (field === 'role') setRole(text);
        ...
        <Text className="text-base mt-1 text-foreground ml-11">
          {field === 'password' ? '••••••••' : 
           field === 'name' ? name : 
           field === 'role' ? role :
           field === 'email' ? email : phone}
    ...
    <View className={`flex-1 ${isDark ? 'dark' : ''} bg-background`}>
      ...
      <ScrollView className="flex-1">
        ...
        <View className="mx-4 rounded-xl bg-card shadow-sm mb-4 px-4">
          {renderProfileItem('Full Name', name, 'name', 'person-circle-outline')}
          {renderProfileItem('Role', role, 'role', 'briefcase-outline')}
          {renderProfileItem('Email Address', email, 'email', 'mail-outline')}
          {renderProfileItem('Phone Number', phone, 'phone', 'call-outline')}
          {renderProfileItem('Password', '••••••••', 'password', 'lock-closed-outline')}
        </View>
